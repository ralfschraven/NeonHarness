...
type ChatActivityStatusKind = 'thinking' | 'working' | 'compacting' | 'retrying'
type ChatActivityNodeStore = { get: (key: string) => { kind: string } | undefined }

/** Collapse live operational nodes into one calm status line in Chat. */
function activityStatusKindFor(
  order: readonly string[],
  nodeStore: ChatActivityNodeStore,
  running: boolean,
): ChatActivityStatusKind | null {
  ...
  for (let index = order.length - 1; index >= 0; index -= 1) {
    ...
    if (node.kind === 'tool-call' || node.kind === 'command') return 'working'
    if (node.kind === 'manual-compaction') return 'compacting'
    if (node.kind === 'model-retry') return 'retrying'
    if (node.kind === 'assistant-step') return 'thinking'
  }
  return 'thinking'
}

function ChatActivityStatus({ kind, t }: { kind: ChatActivityStatusKind; t: ChatViewSlotProps['t'] }) {
  ...
  return (
    <div
      ...
      role="status"
      aria-live="polite"
      aria-label={`${label}. ${t('chat.activityDetails')}`}
      data-chat-activity-status={kind}
    >
      <span>{label}</span>
      <span className={css.turnStatusClock}>{t('chat.activityDetails')}</span>
    </div>
  )
}
...
export function ChatView({
  ...
  const activityStatusKind = useMemo(
    () => activityStatusKindFor(order, nodeStore, running),
    ...
    <div className={css.root}>
      <div ref={listRef} className={css.scroll}>
        <div ref={columnRef} className={css.column} data-chat-flow="">
          ...
          {activityStatusKind !== null && <ChatActivityStatus kind={activityStatusKind} t={t} />}
