...
describe('Hero chrome', () => {
  it('renders the English preview badge through the hero locale seat', () => {
    ...
    expect(view.getByText('Build. Test. Deploy. Reliably.')).toBeTruthy()
    expect(view.getByText('Neon core')).toBeTruthy()
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText('构建。测试。部署。可靠。')).toBeTruthy()
    expect(b.view.getByText('霓虹核心')).toBeTruthy()
  ...
  it('settling phase: a summary that does not prove the session blank hides the composer while it opens', () => {
    ...
    expect(b.view.queryByText('构建。测试。部署。可靠。')).toBeNull()
  })
